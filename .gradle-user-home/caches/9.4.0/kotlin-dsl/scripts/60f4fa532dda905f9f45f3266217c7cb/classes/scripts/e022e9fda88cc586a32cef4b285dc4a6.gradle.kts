...
fun getOrPromptMicrosoftAuthId(): String {
    ...
    if (configFile.exists()) {
        try {
            ...
            val id = properties.getProperty("microsoft.auth.id")
            if (!id.isNullOrBlank()) {
    ...
    if (console != null) {
        ...
        val clientId = console.readLine("Enter your Azure AD client ID (or press Enter to skip): ")?.trim()

        if (!clientId.isNullOrBlank()) {
                                        
            try {
                properties.setProperty("microsoft.auth.id", clientId)
                configFile.parentFile.mkdirs()
                configFile.outputStream().use { properties.store(it, "JMCL Azure AD / Microsoft OAuth Client ID") }
...
tasks.register("configureAzureAd") {
    ...
    doLast {
        ...
        val clientId = console.readLine("Enter new Azure AD client ID (or press Enter to keep current): ")?.trim()
