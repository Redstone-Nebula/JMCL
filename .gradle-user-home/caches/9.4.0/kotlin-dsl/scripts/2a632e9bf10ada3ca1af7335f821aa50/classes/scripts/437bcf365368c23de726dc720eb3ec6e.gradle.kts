...
fun getOrPromptMicrosoftAuthId(): String {
    ...
    if (configFile.exists()) {
        try {
            ...
            configFile.inputStream().use { stream: java.io.InputStream -> props.load(stream) }
            val id: String? = props.getProperty("microsoft.auth.id")
            if (!id.isNullOrBlank()) {
                logger.quiet("Azure AD client ID loaded from {}", configFile)
                return id
            }
        } catch (e: Exception) {
            logger.warn("Failed to read {}, will prompt: {}", configFile, e.message)
        }
    }
    ...
    if (console != null) {
        ...
        val clientId: String? = console.readLine("Enter your Azure AD client ID (or press Enter to skip): ")?.trim()
        ...
        if (!clientId.isNullOrBlank()) {
            ...
            try {
                ...
                val props = java.util.Properties()
                props.setProperty("microsoft.auth.id", clientId)
                configFile.outputStream().use { stream: java.io.OutputStream -> props.store(stream, "JMCL Azure AD / Microsoft OAuth Client ID") }
        ...
        logger.warn("No interactive console available, and no MICROSOFT_AUTH_ID env var or config/azure-ad.properties found.")
        logger.warn("Set MICROSOFT_AUTH_ID environment variable or create config/azure-ad.properties with 'microsoft.auth.id=YOUR_ID'")
    }
...
tasks.register("configureAzureAd") {
    ...
    doLast {
        ...
        val console: java.io.Console? = System.console()
        if (console == null) {
            throw GradleException("No interactive console available. Set MICROSOFT_AUTH_ID env var or manually create config/azure-ad.properties")
        }

        val currentId: String? = try {
            val p = java.util.Properties()
            if (configFile.exists()) {
                configFile.inputStream().use { stream: java.io.InputStream -> p.load(stream) }
                p.getProperty("microsoft.auth.id")
            } else {
                System.getenv("MICROSOFT_AUTH_ID")
            }
        ...
        val clientId: String? = console.readLine("Enter new Azure AD client ID (or press Enter to keep current): ")?.trim()
