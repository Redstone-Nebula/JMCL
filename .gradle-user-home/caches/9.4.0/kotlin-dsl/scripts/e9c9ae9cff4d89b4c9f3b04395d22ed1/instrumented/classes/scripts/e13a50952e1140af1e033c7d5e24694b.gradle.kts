...
private fun patchExeMetadata(data: ByteArray): ByteArray {
    val hmclUtf16 = "HMCL".encodeToByteArray().flatMap { byteArrayOf(it, 0) }.toByteArray()
    val jmclUtf16 = "JMCL".encodeToByteArray().flatMap { byteArrayOf(it, 0) }.toByteArray()
    
    val launcherOld = "HMCLauncher".encodeToByteArray().flatMap { byteArrayOf(it, 0) }.toByteArray()
    val launcherNew = "JMCLauncher".encodeToByteArray().flatMap { byteArrayOf(it, 0) }.toByteArray()
    
    val result = data.copyOf()
    val utf16Hmcl = hmclUtf16.toList()
    val utf16Launcher = launcherOld.toList()
    
    var i = 0
    while (i <= result.size - utf16Hmcl.size) {
        if (result.sliceArray(i until i + utf16Hmcl.size).toList() == utf16Hmcl) {
            jmclUtf16.forEachIndexed { j, b -> result[i + j] = b }
            i += utf16Hmcl.size
        } else if (result.sliceArray(i until i + utf16Launcher.size).toList() == utf16Launcher) {
            launcherNew.forEachIndexed { j, b -> result[i + j] = b }
            i += utf16Launcher.size
        } else {
            i++
        }
    }
