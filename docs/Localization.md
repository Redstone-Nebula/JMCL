...
JMCL provides localization support for multiple languages.

This document describes JMCL's support status for these languages and provides a guide for contributors who want to help with JMCL localization.

## Supported languages

Currently, JMCL supports the following languages:
...
|-----------------------|--------------|----------------------------|------------------------------------|----------------------------------------------------------|----------------|-------------------------------------------|
| English               | `en`         | `default`                  | (empty)                            | `en_us`                                                  | **Primary**    | [cangcang-zh-2025](https://github.com/cangcang-zh-2025)         |
| English (Upside Down) | `en-Qabs`    | `en-Qabs`                  | `en_Qabs`                          | `en_ud`                                                  | Automatic      |                                           |
| Chinese (Simplified)  | `zh-Hans`    | `zh`                       | `_zh`                              | `zh_cn`                                                  | **Primary**    | [cangcang-zh-2025](https://github.com/cangcang-zh-2025)         |
| Chinese (Traditional) | `zh-Hant`    | `zh-Hant`                  | `_zh_Hant`                         | `zh_tw` <br/> `zh_hk`                                    | **Primary**    | [cangcang-zh-2025](https://github.com/cangcang-zh-2025)         |
| Chinese (Classical)   | `lzh`        | `lzh`                      | `_lzh`                             | `lzh`                                                    | Secondary      |                                           |
| Japanese              | `ja`         | `ja`                       | `_ja`                              | `ja_jp`                                                  | Secondary      |                                           |
| Spanish               | `es`         | `es`                       | `_es`                              | `es_es`                                                  | Secondary      | [cangcang-zh-2025](https://github.com/cangcang-zh-2025) |
| Russian               | `ru`         | `ru`                       | `_ru`                              | `ru_ru`                                                  | Secondary      | [cangcang-zh-2025](https://github.com/cangcang-zh-2025) |
| Ukrainian             | `uk`         | `uk`                       | `_uk`                              | `uk_ua`                                                  | Secondary      |                                           |

<details>
<summary>About Language Tags</summary>

JMCL uses language tags that conform to the IETF BCP 47 standard.
...
JMCL will derive a search list from the current language tag and search for resources in order.
...
JMCL requires all pull requests that update documentation and localization resources to also update the resources
...
JMCL welcomes anyone to participate in translation and contribution. 
...
We recommend that contributors submit a [feature request](https://github.com/Open-code-Studio/JMCL/issues/new?template=feature.yml) before providing a new language translation,
...
If you want to add support for a new language to JMCL, please start by translating [`I18N.properties`](../JMCL/src/main/resources/assets/lang/I18N.properties).
The vast majority of JMCL's texts are in this file, and translating it will translate the entire interface.
...
After determining the language tag, please create a file named `I18N_<localization file suffix>.properties` (e.g., `I18N_en.properties`) next to the [`I18N.properties` file](../JMCL/src/main/resources/assets/lang).
...
After translating part of the file, you can [build JMCL yourself](./Contributing.md#build-hmcl), 
and your translations will be included in the compiled JMCL.
...
and JMCL will automatically switch to that language.

At this point, you can push the file to GitHub and submit a PR to JMCL.
The maintainers of JMCL will complete the remaining steps for you.
...
In addition to localized files, JMCL also supports localizing certain text fields in JSON. Localized text in JSON uses the following format:
...
For missing resources in a certain language, JMCL supports a resource fallback mechanism,
...
1. Normalize Language Codes
   ...
   JMCL will try to map it to a registered tag.

   For example, JMCL will replace the language code `eng` with `en`.
...
2. Map Macrolanguages to Individual Languages
   ...
   and the macrolanguage usually refers to a specific individual language, JMCL will replace it with that individual language.

   For example, `zh` (Chinese) usually actually refers to `cmn` (Mandarin), so we replace the language code `zh` with `cmn`.

3. Derive Script

   If the current language tag does not specify a script, JMCL will try to derive the script according to the following rules in order:
...
After refining the language code, JMCL will derive a list of language tags based on this language tag.

For example, for the language tag `en-US`, JMCL will refine it to `en-Latn-US` and derive the following search list:
...
For the language tag `zh-CN`, JMCL will refine it to `cmn-Hans-CN` and derive the following search list:
...
JMCL will merge resources according to the priority of this list; for resources that are difficult to merge (such as font files), 
JMCL will load the highest-priority resource found in this list.

If the current language uses a three-letter ISO 639 code, but there is also a corresponding two-letter code, 
JMCL will map it to the two-letter code before searching for resources.

For example, if the current environment's language tag is `eng-US`, 
JMCL will map it to `en-US` and then search for localization resources according to the above rules.

### Additional Rules for Chinese

JMCL always adds `zh-CN` to the search list for all Chinese environments, 
...
To simplify documentation maintenance, JMCL uses a macro mechanism to automatically maintain parts of the documentation content.
