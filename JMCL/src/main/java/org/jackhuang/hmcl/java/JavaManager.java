 ...
 */
package org.jackhuang.jmcl.java;
...
import org.jackhuang.jmcl.game.JavaVersionConstraint;
...
import org.jackhuang.jmcl.util.CacheRepository;
...
import org.jackhuang.jmcl.util.platform.windows.WinReg;
...
public final class JavaManager {
    ...
    public static final JMCLJavaRepository REPOSITORY = new JMCLJavaRepository(Metadata.GLOBAL_DIRECTORY.resolve("java"));
    public static final JMCLJavaRepository LOCAL_REPOSITORY = new JMCLJavaRepository(Metadata.LOCAL_DIRECTORY.resolve("java"));
    ...
    private static Map<Path, JavaRuntime> searchPotentialJavaExecutables(boolean useCache) {
        Searcher searcher = new Searcher(Metadata.GLOBAL_DIRECTORY.resolve("javaCache.json"));
        if (useCache)
            searcher.loadCache();
        ...
        if (System.getenv("PATH") != null) {
            ...
            for (String path : paths) {
                // https://github.com/Open-code-Studio/JMCL/issues/4079
        ...
        if (System.getenv("JVM-MCL_JRES") != null) {
            String[] paths = System.getenv("JVM-MCL_JRES").split(File.pathSeparator);
            for (String path : paths) {
                try {
