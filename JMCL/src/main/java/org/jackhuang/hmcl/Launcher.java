...
import org.jackhuang.jmcl.setting.SambaException;
import org.jackhuang.jmcl.task.AsyncTaskExecutor;
...
public final class Launcher extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        try {
            ...
            if (Metadata.LOCAL_DIRECTORY.toString().indexOf('=') >= 0) {
    ...
    private static void checkConfigOwner() {
        ...
        if (Files.exists(Metadata.GLOBAL_DIRECTORY))
            files.add(Metadata.GLOBAL_DIRECTORY.toString());
        if (Files.exists(Metadata.LOCAL_DIRECTORY))
            files.add(Metadata.LOCAL_DIRECTORY.toString());
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            LOG.info("JVM-MCL Global Directory: " + Metadata.GLOBAL_DIRECTORY);
            LOG.info("JVM-MCL Current Directory: " + Metadata.LOCAL_DIRECTORY);
            LOG.info("JVM-MCL Jar Path: " + Lang.requireNonNullElse(JarUtils.thisJarPath(), "Not Found"));
            LOG.info("JVM-MCL Log File: " + Lang.requireNonNullElse(LOG.getLogFile(), "In Memory"));
