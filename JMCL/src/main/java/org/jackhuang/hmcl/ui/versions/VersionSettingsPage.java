...
import org.jackhuang.jmcl.game.GameDirectoryType;
import org.jackhuang.jmcl.game.JMCLGameRepository;
import org.jackhuang.jmcl.game.ProcessPriority;
...
import org.jackhuang.jmcl.ui.MemoryStatusBar;
import org.jackhuang.jmcl.ui.WeakListenerHolder;
import org.jackhuang.jmcl.ui.construct.*;
...
import org.jackhuang.jmcl.util.javafx.BindingMapping;
import org.jackhuang.jmcl.util.javafx.PropertyUtils;
import org.jackhuang.jmcl.util.javafx.SafeStringConverter;
import org.jackhuang.jmcl.util.platform.Architecture;
import org.jackhuang.jmcl.util.platform.OperatingSystem;
import org.jackhuang.jmcl.util.platform.SystemInfo;
import org.jackhuang.jmcl.util.platform.hardware.PhysicalMemoryStatus;
import org.jackhuang.jmcl.util.versioning.GameVersionNumber;

import java.util.*;
import java.util.concurrent.atomic.AtomicBoolean;

import static org.jackhuang.jmcl.util.DataSizeUnit.GIGABYTES;
import static org.jackhuang.jmcl.util.DataSizeUnit.MEGABYTES;
import static org.jackhuang.jmcl.util.Pair.pair;
...
public final class VersionSettingsPage extends StackPane implements DecoratorPage, VersionPage.VersionLoadable, PageAware {
    ...
    public VersionSettingsPage(boolean globalSetting) {
        ...
        if (globalSetting) {
            ...
            iconPickerItem.setImage(FXUtils.newBuiltinImage("/assets/img/jvm-mcl.png"));
...

        {
            ...
            VBox maxMemoryPane = new VBox(8);
            {
                ...
                memoryStatusBar.memoryAllocatedProperty().bind(Bindings.createDoubleBinding(() ->
                                (double) JMCLGameRepository.getAllocatedMemory(
                ...
                VBox.setMargin(digitalPane, new Insets(0, 0, 0, 16));
                {
                    ...
                    lblAllocateMemory.textProperty().bind(Bindings.createStringBinding(() -> {
                        ...
                        return i18n(memoryStatus.hasAvailable() && maxMemory > memoryStatus.getAvailable()
                                ...
                                GIGABYTES.convertFromBytes(JMCLGameRepository.getAllocatedMemory(maxMemory, memoryStatus.getAvailable(), chkAutoAllocate.isSelected())),
    ...
    private void initJavaSubtitle() {
        ...
        if (lastVersionSetting == null)
            return;
        initializeSelectedJava();
        JMCLGameRepository repository = this.profile.getRepository();
