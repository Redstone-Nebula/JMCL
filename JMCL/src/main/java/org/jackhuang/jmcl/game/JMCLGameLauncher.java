...
import org.jackhuang.jmcl.auth.AuthInfo;
import org.jackhuang.jmcl.launch.DefaultLauncher;
import org.jackhuang.jmcl.launch.ProcessListener;
import org.jackhuang.jmcl.util.NativePatcher;
import org.jackhuang.jmcl.util.i18n.LocaleUtils;
...
import org.jackhuang.jmcl.util.platform.ManagedProcess;
 ...
 * @author huangyuhui
 */
public final class JMCLGameLauncher extends DefaultLauncher {

    public JMCLGameLauncher(GameRepository repository, Version version, AuthInfo authInfo, LaunchOptions options) {
        this(repository, version, authInfo, options, null);
    }

    public JMCLGameLauncher(GameRepository repository, Version version, AuthInfo authInfo, LaunchOptions options, ProcessListener listener) {
        this(repository, version, authInfo, options, listener, true);
    }

    public JMCLGameLauncher(GameRepository repository, Version version, AuthInfo authInfo, LaunchOptions options, ProcessListener listener, boolean daemon) {
    ...
    private Path extractLwjglUnsafeAgent() throws IOException {
        String agentVersion = JarUtils.getAttribute("jvmmcl.lwjgl-unsafe-agent.version", null);
        if (agentVersion == null) {
            throw new IOException("Missing jvmmcl.lwjgl-unsafe-agent.version attribute");
        }
