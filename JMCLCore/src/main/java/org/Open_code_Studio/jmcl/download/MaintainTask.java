...
import org.Open_code_Studio.jmcl.util.SimpleMultimap;
...
import static org.Open_code_Studio.jmcl.download.LibraryAnalyzer.LibraryType.*;
...
public class MaintainTask extends Task<Version> {
    ...
    private static Version maintainGameWithCpwModLauncher(GameRepository repository, Version version) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library hmclTransformerDiscoveryService = new Library(new Artifact("org.Open_code_Studio.jmcl", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = version.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = version.getLibraries().stream().anyMatch(library -> library.is("org.Open_code_Studio.jmcl", "transformer-discovery-service"));
