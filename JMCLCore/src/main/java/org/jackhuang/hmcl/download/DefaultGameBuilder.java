...
public class DefaultGameBuilder extends GameBuilder {
    ...
    public Task<?> buildAsync() {
        ...
        hints.add(new Task.StagesHint("jmcl.install.game:" + gameVersion));
        hints.add(new Task.StagesHint("jmcl.install.libraries"));
        hints.add(new Task.StagesHint("jmcl.install.assets"));
        ...
        for (Map.Entry<String, String> entry : toolVersions.entrySet()) {
            ...
            hints.add(new Task.StagesHint(String.format("jmcl.install.%s:%s", entry.getKey(), entry.getValue())));
        }

        for (RemoteVersion remoteVersion : remoteVersions) {
            libraryTask = libraryTask.thenComposeAsync(version -> dependencyManager.installLibraryAsync(version, remoteVersion));
            hints.add(new Task.StagesHint(String.format("jmcl.install.%s:%s", remoteVersion.getLibraryId(), remoteVersion.getSelfVersion())));
        }
