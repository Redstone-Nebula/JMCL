...
import org.jackhuang.jmcl.util.SimpleMultimap;
...
import static org.jackhuang.jmcl.download.LibraryAnalyzer.LibraryType.*;
...
public class MaintainTask extends Task<Version> {
    ...
    private static Version maintainGameWithCpwModLauncher(GameRepository repository, Version version) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library hmclTransformerDiscoveryService = new Library(new Artifact("org.jackhuang.jmcl", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = version.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = version.getLibraries().stream().anyMatch(library -> library.is("org.jackhuang.jmcl", "transformer-discovery-service"));
            optiFine.ifPresent(library -> {
                builder.addJvmArgument("-Djmcl.transformer.candidates=${library_directory}/" + library.getPath());
                ...
                try (InputStream input = MaintainTask.class.getResourceAsStream("/assets/game/JMCLTransformerDiscoveryService-1.0.jar")) {
                    Files.createDirectories(libraryPath.getParent());
                    Files.copy(Objects.requireNonNull(input, "Bundled JMCLTransformerDiscoveryService is missing."), libraryPath, StandardCopyOption.REPLACE_EXISTING);
                } catch (IOException | NullPointerException e) {
                    LOG.warning("Unable to unpack JMCLTransformerDiscoveryService", e);
                }
            });
        }
