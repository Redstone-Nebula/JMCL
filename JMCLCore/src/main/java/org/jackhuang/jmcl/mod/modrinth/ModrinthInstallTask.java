 ...
 */
package org.jackhuang.jmcl.mod.modrinth;
...
public class ModrinthInstallTask extends Task<Void> {
    ...
    public ModrinthInstallTask(DefaultDependencyManager dependencyManager, Path zipFile, Modpack modpack, ModrinthManifest manifest, String name, String iconUrl) {
        ...
        for (Map.Entry<String, String> modLoader : manifest.getDependencies().entrySet()) {
            switch (modLoader.getKey()) {
                ...
                case "forge":
                    ...
                    break;
                case "neoforge":
                // https://github.com/Open-code-Studio/JMCL/pull/5170
                case "neo-forge":
        ...
        dependents.add(new ModpackInstallTask<>(zipFile, run, modpack.getEncoding(), subDirectories, any -> true, config).withStage("jmcl.modpack"));
        dependents.add(new MinecraftInstanceTask<>(zipFile, modpack.getEncoding(), subDirectories, manifest, ModrinthModpackProvider.INSTANCE, manifest.getName(), manifest.getVersionId(), repository.getModpackConfiguration(name)).withStage("jmcl.modpack"));
